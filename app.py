...
StockSense App
    ...
    if data is not None:
        stock_alerts = generate_stock_alerts(data, threshold=stock_threshold)
        expiry_alerts = generate_expiry_alerts(data, days_until_expiry=expiry_days)
        ...
        if total_alerts == 0:
            ...
            st.warning(f"{total_alerts} alerts found.")

            for alert in stock_alerts + expiry_alerts:
                sent = send_pushover_notification(PUSHOVER_USER_KEY, PUSHOVER_API_TOKEN, alert)
                if sent:
                    st.success(f"Pushover sent: {alert}")
                else:
                    st.error(f"❌ Failed to send: {alert}")
    else:
